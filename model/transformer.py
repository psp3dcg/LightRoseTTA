...
def _get_clones(module, N):
    '''
    Make N copies of the module

        Input:
            - module(nn.Module): input module
            - N(int): the number of copies
        Output:
            - module_list(nn.ModuleList): module copies
        '''
...
class FeedForwardLayer(nn.Module):
    def __init__(self, d_model, d_ff, p_drop=0.1):
        ...
        Input:
            - d_model(int): the dimension of model feature
            - d_ff(int): the dimension of hidden feature in feed forward network
            - eps(float): a very small number
        '''
    ...
    def forward(self, src):
        '''
        Input:
            - src(tensor): input feature
        Output:
            - src(tensor): output feature
        '''
...
class MultiheadAttention(nn.Module):
    def __init__(self, d_model, heads, k_dim=None, v_dim=None, dropout=0.1):
        ...
        Input:
            ...
            - k_dim(int): the dimension of key
            - v_dim(int): the dimension of value
    ...
    def forward(self, query, key, value, return_att=False):
        ...
        Input:
            ...
            - key(tensor): input key
            - value(tensor): input value  
            - return_att(bool): return attetion or not
        Output:
            - out(tensor): updated value
        '''
...
class DirectMultiheadAttention(nn.Module):
    def __init__(self, d_in, d_out, heads, dropout=0.1):
        '''
        Input:
            - d_in(int): the dimension of input feature 
            - d_out(int): the dimension of output feature 
            - heads(int): the number of attention heads
            - dropout(float): dropout ratio
        '''
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, d_ff, heads, p_drop=0.1, performer_opts=None, use_tied=False):
        ...
        Input:
            - d_model(int): the dimension of model feature 
            - d_ff(int): the dimension of feed forward network feature
            - heads(int): the number of attention heads
            - p_drop(float): dropout ratio
            - performer_opts(dict): performr options 
            - use_tied(bool): use soft tied multi-head attention or not
        '''
    ...
    def forward(self, src, return_att=False):
        ...
        Input:
            - src(tensor): input feature
            - return_att(bool): return attention or not
        Output:
            - src(tensor): updated feature
        '''
...
class AxialEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, d_ff, heads, p_drop=0.1, performer_opts=None,
        ...
        Input:
            ...
            - p_drop(float): dropout ratio
            - performer_opts(dict): performr options 
            - use_tied_row(bool): use tied multi-head attention or not along the row
            - use_tied_col(bool): use tied multi-head attention or not along the column
            - use_soft_row(bool): use soft tied multi-head attention or not along the row
        '''
...
class Encoder(nn.Module):
    def __init__(self, enc_layer, n_layer):
        '''
        Input:
            - enc_layer(nn.Module): encoder layer
...
class CrossEncoderLayer(nn.Module):
    def __init__(self, d_model, d_ff, heads, d_k, d_v, performer_opts=None, p_drop=0.1):
        ...
        Input:
            ...
            - d_ff(int): the dimension of hidden feature in feed forward network
            - heads(int): the number of attention heads 
            - d_k(int): the dimension of key
            - d_v(int): the dimension of value
            - performer_opts(dict): performer options
            - p_drop(float): dropout ratio
        '''
    ...
    def forward(self, src, tgt):
        ...
        Input:
            - src(tensor): source feature 
            - tgt(tensor): target feature
        Output:
            - tgt(tensor): updated target feature
        '''
...
class DirectEncoderLayer(nn.Module):
    def __init__(self, heads, d_in, d_out, d_ff, symmetrize=True, p_drop=0.1):
        ...
        Input:
            ...
            - d_in(int): the dimension of input feature 
            - d_out(int): the dimension of output feature 
            - d_ff(int): the dimension of hidden feature 
            - symmetrize(bool): symmetrize the input feature or not
            - p_drop(float): dropout ratio
        '''
...
class CrossEncoder(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        Input:
            ...
            - tgt(tensor): target feature
        Output:
            - output(tensor): updated target feature
        ''' 
        output = tgt
        for layer in self.layers:
