...
tr_params = {
    "SG"     : np.array([[[-21,14,39,54,59,54,39,14,-21]]])/231,
    "DCUT"    : 19.5,
    "ALPHA"   : 1.57,
...
def get_ang(a, b, c):
    ...
    Input:
        - a(tensor): a set storing Cartesian coordinates of atoms
        - b(tensor): b set storing Cartesian coordinates of atoms
        - c(tensor): c set storing Cartesian coordinates of atoms

    Output:
        - angle(pytorch tensors): angle of edge ab and bc
    """
    v = a - b
    w = c - b
...
def get_dih(a, b, c, d):
    ...
    Input:
        ...
        - b(tensor): b set storing Cartesian coordinates of atoms
        - c(tensor): c set storing Cartesian coordinates of atoms
        - d(tensor): d set storing Cartesian coordinates of atoms
    Output:
        - dih(pytorch tensors): pytorch tensor of shape [batch,nres]
...
# translation and rotation for refinement
class TR_Fold():

    def __init__(self, pred):
        """    
        Input:
            - pred(tensor): inter-residue distance and orientations
        ...
        # backbone motif 
        # backbone atom structure template
    ...
    def akima(self, y,h):
        ...
        Input:
            - y(tensor): inter-residue distance or orientation
            - h(int): steps for distance or orientation
        Output:
            - coef(tensor): akima spline coefficient
        """
        m = (y[:,1:]-y[:,:-1])/h
    ...
    def fold(self, xyz, batch=15, lr=0.8, nsteps=1000):
        """ 
        Akima spline coefficients (boundaries trimmed to [2:-2])
        ...
        for step in range(nsteps):
            ...
            loss_peptide_dihedral = (torch.abs(get_dih(Ca[:,:-1],C[:,:-1],N[:,1:],Ca[:,1:])) - 3.0511)**2



            #
            loss_geom = loss_nc.mean(1) + loss_cacn.mean(1) + loss_canc.mean(1) + 2*loss_peptide_dihedral.mean(1)
        ...
        update_coord = bb[minidx].permute(1,0,2) 
