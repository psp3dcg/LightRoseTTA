...
from model.transformer import AxialEncoderLayer
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, p_drop=0.1, max_len=5000):
        '''
        Residue index embedding in sequence
    ...
    def forward(self, x, idx_s):
        '''
        Input:
            - x(tensor): hidden feature dimension
            - idx_s(tensor): 
        Output:
            - x(tensor):
        '''
...
class MSA_emb(nn.Module):
    def __init__(self, d_model=64, d_msa=21, p_drop=0.1, max_len=5000):
        '''
        Make the MSA embedding

        Input:
            - d_model(int): hidden feature dimension
            - d_msa(int): the dimension of input MSA embedding
            - p_drop(float): dropout ratio
            - max_len(int): residue index from ground truth pdb
        '''
    ...
    def forward(self, msa, idx):
        ...
        Output:
            - out(tensor): updated MSA embedding
        '''
        B, N, L = msa.shape
...
class Templ_emb(nn.Module):
    def __init__(self, d_t1d=3, d_t2d=10, d_templ=64, n_att_head=4, r_ff=4,
        ...
        '''
        Make the template embedding

        Input:
            - d_t1d(int): the dimension of 1D template info
            - d_t2d(int): the dimension of 2D template info
            - d_templ(int):  the dimension of template embedding
...
class Pair_emb_w_templ(nn.Module):
    def __init__(self, d_model=128, d_seq=21, d_templ=64, p_drop=0.1):
        '''
        Pair embedding with template info

        Input:
            - d_model(int): the dimension of hidden feature 
            - d_seq(int): the dimension of sequence one-hot embedding
            - d_templ(int): the dimension of template embedding
            - p_drop(float): dropout ratio
        '''
    ...
    def forward(self, seq, idx, templ):
        '''     
        Input:
            - seq(tensor): residue sequence one-hot vector
            - idx(tensor): residue index
            - templ(tensor): template embedding
        Output:
            - final_pair(tensor): updated pair embedding
        '''
        # input:
...
class Pair_emb_wo_templ(nn.Module):
    ...
    def __init__(self, d_model=128, d_seq=21, p_drop=0.1):
        '''
        Pair embedding without template info
